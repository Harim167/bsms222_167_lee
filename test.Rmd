...
We can also ssign values using `=` instead of `<=`, but we recommend against using `=` to avoid confusion. 
Copy and paste the code above into your console to define the three variables. Note that R does not print anything when we make this assignment. This means the objects were deifined successfully. Had you made a mistake, you would have received an error message.
...
Once you define variables, the data analysis process can usually be described as a series of functions applied to the data. R inclues several predefined functions and most of the analysis pipelines we construct make extensive use of these. 

We already used the `install.packages`, `library`, and `ls` functions. We also used the function `sqrt` to solve the quadratic equation above. There are many more prebuilt functions and even more can be added through packages. These functions do not appear in the workspace because you did not deine them, but they are available for immediate use. 
...
#> seq(from, to), seq {base}
help(sum)
#> sum returns the sum of all the values present in its arguments.

# answer: 'seq' creates a list of numbers and 'sum' adds them up.(b)
          # 'seq' doesn't create a 'random' list. It is regular.
```

4. In math and programming, we say that we evaluate a function when we replace the argument with a given number. So if we type `sqrt(4)`, we evaluate the `sqrt` function. In R, you can evaluate a function inside another function. The evaluations lhappen from the inside out. Use one line of code to compute the log, in base 10, of the square root of 100.
```{r}
...
#answer) I think there is not an answer which always return the numeric value stored in `x`.
...
```
This tells us much more about the object. We see that the table has 51 rows (50 status plus DC) and five variables. We can show the first six lines using the function `head`.
```{r}
head(murders)
...
Before we go any further in answering our original question about different states, let's learn more about he components of this object.
...
# I got 5 variables; state, abb, region, population, and total. So the answer is c. 
```
