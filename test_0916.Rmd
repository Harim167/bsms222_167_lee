...
```{r}
h_rank <- rank(-murders$rate)
murders <- mutate(murders, h_rank)
```
...
```{r}
filter(murders, h_rank == "1")
filter(murders, h_rank == "2")
filter(murders, h_rank == "3")
filter(murders, h_rank == "4")
filter(murders, h_rank == "5")
```
...
```{r}
nw_table <- select(murders_nw, state, rate, h_rank)
filter(nw_table, rate < 1)
```
...
my_states <- murders %>% mutate(rate = total / population * 100000, rank = rank(-rate)) %>% filter(region %in% c("Northeast", "West")) %>% select(state, rate, rank)
my_states
```
